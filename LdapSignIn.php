...
class LdapSignIn extends WireData implements Module, ConfigurableModule
{
    ...
    protected $host, $port, $defaultLoginDomain, $userDefaultRoles;
    ...
    public function init()
    {
        if (isset($this->data['host'])) {
            $host = explode(':', $this->data['host']);
            $this->host = $host[0];
            $this->port = empty($host[1]) ? 389 : $host[1];
        }

        if (isset($this->data['loginDomain'])) $this->defaultLoginDomain = $this->data['loginDomain'];
    ...
    {
        if ($event->return) return; // Skip if already logged in

        $username = $event->arguments[0];
        $password = $event->arguments[1];

        // skip if user already exists locally
        if (!$this->users->get("name=$username") instanceof NullPage) return;
        ...
        if ($this->ldapUserLogin($username, $password)) {
            ...
            if ($user instanceof NullPage) {
                ...
                $user->pass = $password;
                foreach ($this->userDefaultRoles as $role) $user->addRole($role);
                $user->save();
            } else {
                $user->pass = $password;
                $user->save();
    ...
    static public function getModuleConfigInputfields(array $data)
    {
        ...
        if (isset($data['host'])) $hostField->value = $data['host'];
        $hostField->description = __('The hostname of your LDAP server. This can be either an ip address or a domain name. Supply a custom port (different than 389) separated with a column. Examples: 10.0.0.1, controller.domain.com, controller.domain.com:388');
        ...
        if (isset($data['loginDomain'])) $defaultLoginDomainField->value = $data['loginDomain'];
        ...
        if (isset($data['userDefaultRoles'])) $userDefaultRolesField->value = $data['userDefaultRoles'];
